...
def save_frame_log(
    ...
    fieldnames = [
        ...
        "yaw", "pitch", "roll",
        "pose_valid",
        "raw_crop_cx_rot", "raw_crop_cy_rot", "raw_crop_w_rot", "raw_crop_h_rot",
        "crop_cx_rot", "crop_cy_rot", "crop_w_rot",
        "crop_h_rot",
        "stable_crop_cx_rot", "stable_crop_cy_rot", "stable_crop_w_rot", "stable_crop_h_rot",
        "eye_dist", "eye_mouth_dist", "scale_deviation_ratio",
        "delta_cx", "delta_cy",
    ...
    with open(output_path, "w", newline="") as f:
        ...
        for fd in frame_data:
            ...
            row = {
                ...
                "raw_crop_cx_rot": round(fd.raw_crop_cx_rot, 2) if fd.raw_crop_cx_rot is not None else "",
                "raw_crop_cy_rot": round(fd.raw_crop_cy_rot, 2) if fd.raw_crop_cy_rot is not None else "",
                "raw_crop_w_rot": round(fd.raw_crop_w_rot, 2) if fd.raw_crop_w_rot is not None else "",
                "raw_crop_h_rot": round(fd.raw_crop_h_rot, 2) if fd.raw_crop_h_rot is not None else "",
                ...
                "crop_h_rot": round(fd.crop_h_rot, 2) if fd.crop_h_rot is not None else "",
                "stable_crop_cx_rot": round(fd.stable_crop_cx_rot, 2) if fd.stable_crop_cx_rot is not None else "",
                "stable_crop_cy_rot": round(fd.stable_crop_cy_rot, 2) if fd.stable_crop_cy_rot is not None else "",
                "stable_crop_w_rot": round(fd.stable_crop_w_rot, 2) if fd.stable_crop_w_rot is not None else "",
                "stable_crop_h_rot": round(fd.stable_crop_h_rot, 2) if fd.stable_crop_h_rot is not None else "",
                "eye_dist": round(fd.eye_dist, 2) if fd.eye_dist is not None else "",
                "eye_mouth_dist": round(fd.eye_mouth_dist, 2) if fd.eye_mouth_dist is not None else "",
                "scale_deviation_ratio": round(fd.scale_deviation_ratio, 4) if fd.scale_deviation_ratio is not None else "",
