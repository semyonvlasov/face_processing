#!/usr/bin/env python3
"""Service launcher for TalkVid raw-Drive processing over shared process defaults."""
...
EXPECTED_STAGE = "process_talkvid_raw_from_gdrive"
...
def main() -> int:
    ...
    args = parse_args(paths.stage_root / "configs" / "default_gdrive.yaml")
    ...
    try:
        ...
        cmd = [
            python_bin,
            str(paths.stage_root / "gdrive_processor.py"),
            "--source-folder-id",
            raw_folder_id,
            "--dest-folder-id",
            processed_folder_id,
