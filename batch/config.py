"""Shared YAML config loading and stage-launch helpers for batch processing."""
...
def discover_stage_paths(launcher_file: str) -> StagePaths:
    """Discover paths relative to a launcher script.

    In the new layout, launchers live at ``batch/launchers/<script>.py``.
    * ``script_dir``           -> ``batch/launchers/``
    * ``stage_root``           -> ``batch/``  (parent of launchers)
    * ``repo_root``            -> parent of ``batch/``
    * ``dataset_prepare_root`` -> same as ``repo_root`` (no separate dataset_prepare dir)
    """
    launcher_path = Path(launcher_file).resolve()
    script_dir = launcher_path.parent          # batch/launchers/
    stage_root = script_dir.parent             # batch/
    repo_root = stage_root.parent              # <repo_root>
    dataset_prepare_root = repo_root           # no separate dataset_prepare directory
    return StagePaths(
